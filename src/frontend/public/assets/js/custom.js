(function ($) {
  ...
  function onScroll(event) {
    ...
    $('.nav a').each(function () {
      ...
      // Skip non-anchor links (e.g., external paths like /login)
      if (!href || href[0] !== '#') {
        return; // Skip processing this link
      }

      var refElement = $(href); // This assumes href is a valid ID selector
      if (
        refElement.length && // Ensure the element exists
